...
// Global variable created by Unnar Thor Bachmann.
...
var resizePizzas = function(size) {
  ...
  function determineDx (elem, size) {
    ...
    /* First optimzation.
      Almost the original code gone.
      The function returns percentage value
      instead of pixile value.
    */
    switch(size) {
      case "1":
        return 25;
  ...
  /* Second optimization.
    Calculating only the dom elements with class randomPizzaContainer
    once. Then calculating dx and newwidth only once.
    Therefore minimizing the amount of work done in the for-loop.*/
    var cont = document.getElementsByClassName('randomPizzaContainer');
    var dx = determineDx(cont[0], size);
    var newwidth = dx + '%';
    for (var i = 0; i < cont.length; i++) {
         cont[i].style.width = newwidth;
    }
  }
...
/* Second optimization.
This class of dom elements was moved out of the updatePositions function
in order to not recalculate them when updating them with
updatePositions.*/
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  // Variable created by Unnar Thor Bachmann.
